...
describe('Access rights helper test', function () {
    ...
    it("Default (CRUD) tokens created for every entity test", function () {
        ...
        (0, chai_1.expect)(tokensAmount).to.equal(4 * (entitiesAmount + formsAmount) + 2 + 1 + 1); // 4 for every entity and form + 2 routes for migrations +1 for navigation +1 for mediamanager
    });
});
