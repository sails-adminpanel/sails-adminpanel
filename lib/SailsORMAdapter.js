...
/**
 * Преобразует атрибуты Waterline в формат Adminizer
 */
function mapWaterlineToAdminizerAttributes(model) {
	...
	// Добавляем ассоциации из meta, если они есть
...
/**
 * Определяет тип атрибута на основе типа Waterline
 */
...
export class WaterlineModel extends AbstractModel {
	/** @type {Object} Настоящая Waterline модель (коллекция) */
	model;

	/**
	 * @param {string} modelName - Имя модели в Adminizer
	 * @param {Object} model - Настоящая Waterline модель из sails.models
	 */
	constructor(modelName, model) {
		...
		// Проверяем, что это настоящая Waterline модель с методами
		...
		// Извлекаем primaryKey
		let primaryKey = 'id';
		if (model.primaryKey) {
			primaryKey = model.primaryKey;
		} else {
			// Ищем primary key в атрибутах
		...
		// Сохраняем прямую ссылку на модель
		this.model = model;
	}

	/**
	 * Преобразует критерии Adminizer в формат Waterline
	 */
	...
	_convertAdminizerCriteriaToWaterlineOptions(criteria = {}) {
		...
		// Waterline принимает where напрямую, не вложенным
		if (Object.keys(waterlineWhere).length > 0) {
			Object.assign(waterlineCriteria, waterlineWhere);
		}
	...
	async _applyPopulate(query, populate) {
		if (populate && typeof populate === 'object') {
			...
			// Автоматически populate все ассоциации
	...
	async _findOne(criteria = {}) {
		console.log('_findOne criteria:', JSON.stringify(criteria, null, 2));

		const { waterlineCriteria, populate } = this._convertAdminizerCriteriaToWaterlineOptions(criteria);
		console.log('_findOne waterlineCriteria:', JSON.stringify(waterlineCriteria, null, 2));
...
export class SailsORMAdapter extends AbstractAdapter {
	 ...
	 * Ищет настоящую Waterline модель в sails.models или в переданном orm
	 */
	getModel(modelName) {
		// Сначала ищем в this.orm (это может быть sails.models)
		let model = this.orm[modelName.toLowerCase()];

		// Если не нашли, пробуем найти в global.sails.models
		if (!model && global.sails && global.sails.models) {
			model = global.sails.models[modelName.toLowerCase()];
		}

		// Если нашли, но это не Waterline модель с методами,
		// ищем настоящую модель в global.sails.models
		if (model && typeof model.findOne !== 'function') {
			if (global.sails && global.sails.models) {
				// Ищем по атрибутам (если объект тот же)
				for (const [name, m] of Object.entries(global.sails.models)) {
					if (m === model || m.attributes === model.attributes) {
						if (typeof m.findOne === 'function') {
							console.log(`Found Waterline model by attributes match: ${name}`);
							return m;
						}
					}
				}

				// Ищем по имени (игнорируя суффиксы)
				const baseName = modelName.toLowerCase().replace(/ap$/i, '');
				for (const [name, m] of Object.entries(global.sails.models)) {
					if (typeof m.findOne === 'function') {
						const mBaseName = name.toLowerCase().replace(/ap$/i, '');
						if (mBaseName === baseName) {
							console.log(`Found Waterline model by base name match: ${name}`);
							return m;
						}
					}
				}
