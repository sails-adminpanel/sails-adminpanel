...
class AbstractMediaManager {
     ...
     * @param imageSizes
     */
    upload(file, filename, origFileName) {
    ...
    createVariants(item, file, parent, filename, imageSizes) {
        const parts = item.mimeType.split("/");
        return this.getItemType(parts[0])?.createVariants(file, parent, filename, imageSizes);
    }
    /**
     ...
     * @param config
     */
    uploadVariant(item, file, fileName, config) {
        const parts = item.mimeType.split("/");
        return this.getItemType(parts[0])?.uploadVarinat(item, file, fileName, config);
    }
    /**
