...
export default function bindMediaManager() {
	sails.after(["hook:orm:loaded"], async () => {
		try{
			let mediaManager = new DefaultMediaManager('default', 'media-manager', `${process.cwd()}/.tmp/public/media-manager/`, 'mediamanagerap', 'mediamanagermetaap', 'mediamanagerassociationsap')
