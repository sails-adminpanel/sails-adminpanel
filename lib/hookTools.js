				...
				Object.entries(models).forEach(([modelName, modelDef]) => {
					...
					Object.entries(originalModel).forEach(([key, value]) => {
						if (typeof value === 'function') {
							...
							// adminizer's column-name mapping is `field` (see its
							// typeorm/sequelize adapters: attr.field || attr.columnName);
							// waterline expects `columnName`
							if (attributes[key].field) {
								attributes[key].columnName = attributes[key].field;
								delete attributes[key].field;
							}
