...
function bindRoutes() {
    ...
    if (config.globalSettings.enableWidgets) {
        sails.router.bind(`${config.routePrefix}/widgets-get-all`, (0, bindPolicies_1.default)(policies, widgets_1.default));
        if (config.widgets.switchers.length) {
            for (const switcher of config.widgets.switchers) {
                if (switcher.controller) {
                    ...
                    sails.router.bind(`${config.routePrefix}/${switcher.api}`, (0, bindPolicies_1.default)(policies, switcherController.default));
                }
            }
        }
    }
