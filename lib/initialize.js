...
async function default_1(sails, cb) {
    ...
    await migrationsHelper_1.MigrationsHelper.addToProcessMigrationsQueue(`${sails.config.adminpanel.rootPath}/migrations`);
    // run project migrations
    if (process.env.AUTO_MIGRATIONS) {
        let result = await migrationsHelper_1.MigrationsHelper.processSpecificDirectoryMigrations(sails.config.adminpanel.migrations.path, "up");
