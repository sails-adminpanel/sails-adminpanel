...
import {MediaManagerApp} from "../apps/media-manager/MediaManagerApp.mjs";
import {NavigationApp} from "../apps/navigation/NavigationApp.mjs";
...
export default async function initialize(cb) {
	...
	sails.on('hook:orm:loaded', async function () {
		...
		await adminizer.appManager.enable(new MediaManagerApp({
			...sails.config.adminpanel.mediamanager
		}))
		console.log(chalk.default.green('✅  Media Manager enabled!'));

		// Enable Navigation
		await adminizer.appManager.enable(new NavigationApp({
			...sails.config.adminpanel.navigation
		}))
		console.log(chalk.default.green('✅  Navigation enabled!'));
