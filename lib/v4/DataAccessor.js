...
class DataAccessor {
                ...
                if (combinedFieldConfig === false) {
                    ...
                    if (typeof combinedFieldConfig === "object") {
                        ...
                        if (combinedFieldConfig.groupsAccessRights) {
                            ...
                            ignoreField = this.user.groups.some((group) => group.name.toLowerCase() === "default user group");
                        }
                    }
        ...
        Object.entries(Model.attributes).forEach(([key, modelField]) => {
            ...
            if (hasAccess) {
                associatedFields[key] = {
                    config: configHelper_1.ConfigHelper.normalizeFieldConfig(fieldConfig, key, modelField),
                    model: modelField,
        ...
        for (const fieldKey in record) {
            ...
            if (this.checkFieldAccess(fieldConfig.config)) {
                ...
                if (fieldConfig.config.type !== 'association' && fieldConfig.config.type !== 'association-many') {
                    ...
                    filteredRecord[fieldKey] = fieldValue.map(associatedRecord => this.filterAssociatedRecord(associatedRecord, fieldConfig.populated));
                }
    ...
    sanitizeUserRelationAccess(criteria) {
