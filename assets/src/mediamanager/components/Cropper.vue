	...
	<div class="px-12 py-14 max-w-[1440px] w-full mx-auto overflow-y-auto h-full" ref="cropperWrapperRef">
		...
		<div class="grid grid-cols-4 gap-2 mt-4">
			<div class="flex flex-col gap-2" v-for="(value, key, i) in coord">
				<label class="admin-panel__title capitalize"
					   for="author">{{ key }}</label>
				<input class="text-input w-full h-[20px]" type="number" :id="key" :name="key" v-model.trim="coord[key]"
		...
		<div class="grid grid-cols-4 gap-2 mt-4">
			...
			<button type="button"
					class="flex justify-center items-center text-gray-900 bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-100 font-medium rounded text-sm px-5 py-2.5 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700"
					@click="vueCropperRef?.move(0, 20)">
				<svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 rotate-180" viewBox="0 0 24 24">
					<path fill="currentColor" d="M13 20h-2V8l-5.5 5.5l-1.42-1.42L12 4.16l7.92 7.92l-1.42 1.42L13 8z"/>
				</svg>
			</button>
			<button type="button"
				...
				<svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 -rotate-90" viewBox="0 0 24 24">
					<path fill="currentColor" d="M13 20h-2V8l-5.5 5.5l-1.42-1.42L12 4.16l7.92 7.92l-1.42 1.42L13 8z"/>
				</svg>
			</button>
			<button type="button"
		...
		<div class="grid grid-cols-4 gap-2 mt-4">
			...
			<button class="btn btn-green btn-text" @click="save">
				Save
			</button>
		</div>
...
function preview() {
	...
	vueCropperRef.value.getCroppedCanvas().toBlob((blob) => {
