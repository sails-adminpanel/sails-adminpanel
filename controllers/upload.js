...
let async = require('async'); // узнать как тут убрать async, там функции на колбэках (резайзы)
// выполняются параллельно другой функции
...
function upload(req, res) {
    ...
    //                             i.quality = 60;
    //                         }
    //                         resizes.push(function (callback) {
    ...
    //                         });
    //                     });
    //                     async.parallel(resizes, function (err, results) {
    //                         if (err) return res.serverError(err);
    //
